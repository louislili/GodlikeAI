...
## 核心
- 让机器像神一样思考，是一个超级人工智能方法论。现在的AI都在训练得越来越像人，如人，超越人。那么，这条路的未来，到底是什么？神会出现吗，神有限制吗，神的存在是否会改变我们的世界？

<img src="gpt-n.png" alt="Godlike AI" width="100%" />
...
### 口碑单句打标签
- 简介：对单条口碑文本进行结构化打标（情感、主题、风险、实体类型等），便于后续聚合与分析。
- 文件：
  - 工作流定义：`workflows/fastgpt/口碑单句打标签.json`
  - 介绍文档：`workflows/fastgpt/口碑单句打标签.md`
- 输入/输出：输入 `userChatInput`（必填）与可选 `userFiles`；输出结构化标签 JSON（含情感/主题/实体/风险）。
- 使用建议：配置标签集合、阈值与词典；将结果写入数据仓库用于报表与检索。

### 口碑拆句
- 简介：将长文本口碑拆分为更细粒度的句子片段，提升后续打标与意图识别效果。
- 文件：
  - 工作流定义：`workflows/fastgpt/口碑拆句.json`
  - 介绍文档：`workflows/fastgpt/口碑拆句.md`
- 输入/输出：输入 `userChatInput`（长文本）；输出句子数组与可选位置信息。
- 使用建议：配置最小句长与停用词；作为打标签/意图分类的前置步骤。

### 口碑数据分析助手
- 简介：对口碑数据进行趋势、主题与风险分析，生成结构化洞察与溯源引用。
- 文件：
  - 工作流定义：`workflows/fastgpt/口碑数据分析助手.json`
  - 介绍文档：`workflows/fastgpt/口碑数据分析助手.md`
- 输入/输出：输入分析指令或数据片段；输出指标、洞察与预警（JSON）。
- 使用建议：配置数据源与指标口径；与拆句/打标签串联形成闭环分析。
